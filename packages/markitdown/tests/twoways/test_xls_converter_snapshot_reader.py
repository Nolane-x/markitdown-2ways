...
SOURCE = b"\xd0\xcf\x11\xe0\xa1\xb1\x1a\xe1H28 derived XLS fixture"
INFO = StreamInfo(
    extension=".xls",
...
def _snapshot(
    *,
    sheets: tuple[XlsSheetMarkdownSnapshot, ...] = SHEETS,
    provider: str = "offline-xls-materializer",
) -> XlsConverterSnapshot:
    return XlsConverterSnapshot(
        sheets=sheets,
        provider=provider,
        materialization_id="workbook-001",
        table_provider="fake-pandas-xlrd",
...
def test_reader_binds_source_ordered_sheets_and_exact_projection() -> None:
    ...
    assert document.canvases[0].kind == "derived-xls-workbook"
    assert document.canvases[0].native_locator is None

    assert document.source is not None
    assert document.source.format == "xls-converter-derived-source"
    assert document.source.filename == "fixture.xls"
    assert document.source.mimetype == "application/vnd.ms-excel"
    assert document.source.sha256 == sha256(SOURCE).hexdigest()
    assert document.source.size_bytes == len(SOURCE)

    evidence = document.metadata.custom["twoways.xls_converter_snapshot.v1"]
    ...
    assert evidence["provider"] == "offline-xls-materializer"
    assert evidence["materialization_id"] == "workbook-001"
    assert evidence["table_provider"] == "fake-pandas-xlrd"
    ...
    assert evidence["cfb_biff_parsing_performed_by_twoways"] is False
    assert evidence["pandas_executed_by_twoways"] is False
    assert evidence["xlrd_executed_by_twoways"] is False
...
def test_per_sheet_strip_and_final_strip_are_exact(
    ...
    document = read_xls_converter_snapshot_ir(
        ...
        stream_info=INFO,
        snapshot=_snapshot(
            sheets=(XlsSheetMarkdownSnapshot(name="Only", markdown=markdown),)
        ),
    )
...
def test_sheet_order_is_identity_bearing_and_preserved() -> None:
    first = _snapshot(
        sheets=(
            XlsSheetMarkdownSnapshot(name="A", markdown="alpha"),
            XlsSheetMarkdownSnapshot(name="B", markdown="beta"),
        )
    )
    second = _snapshot(
        sheets=(
    ...
    first_doc = read_xls_converter_snapshot_ir(
        BytesIO(SOURCE), stream_info=INFO, snapshot=first
    )
    second_doc = read_xls_converter_snapshot_ir(
...
def test_root_is_derived_and_h17_remains_native_authority() -> None:
    document = read_xls_converter_snapshot_ir(
        BytesIO(SOURCE),
        stream_info=INFO,
        snapshot=_snapshot(),
    ...
    assert decision.reason_code == "xls.output.not_native_writable"
...
def test_source_and_sheet_content_are_independent_identity_authorities() -> None:
    baseline = read_xls_converter_snapshot_ir(
        BytesIO(SOURCE), stream_info=INFO, snapshot=_snapshot()
    )
    changed_source = read_xls_converter_snapshot_ir(
        BytesIO(SOURCE + b"x"), stream_info=INFO, snapshot=_snapshot()
    )
    changed_sheet = read_xls_converter_snapshot_ir(
        BytesIO(SOURCE),
        stream_info=INFO,
        snapshot=_snapshot(
            sheets=(
                XlsSheetMarkdownSnapshot(name="Summary", markdown="changed"),
                SHEETS[1],
            )
        ),
    )
...
def test_repeated_reads_and_canonical_round_trip_are_deterministic() -> None:
    first = read_xls_converter_snapshot_ir(
        BytesIO(SOURCE), stream_info=INFO, snapshot=_snapshot()
    )
    second = read_xls_converter_snapshot_ir(
        BytesIO(SOURCE), stream_info=INFO, snapshot=_snapshot()
    )
...
@pytest.mark.parametrize(
    ("info", "accepted_by"),
    (
        (StreamInfo(extension=".xls"), "extension"),
        (StreamInfo(extension=".XLS"), "extension"),
        (StreamInfo(mimetype="application/vnd.ms-excel"), "mimetype"),
        (StreamInfo(mimetype="application/excel"), "mimetype"),
        (
            StreamInfo(mimetype="application/vnd.ms-excel; charset=binary"),
            "mimetype",
        ),
    ),
)
...
def test_outside_explicit_surface_fails_closed(info: StreamInfo) -> None:
    with pytest.raises(ValueError, match="XlsConverter"):
        read_xls_converter_snapshot_ir(
            BytesIO(SOURCE),
            stream_info=info,
            snapshot=_snapshot(),
