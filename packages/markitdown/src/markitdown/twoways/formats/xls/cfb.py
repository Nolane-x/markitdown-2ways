...
def _sector_offset(sector_id: int, sector_size: int, sector_count: int) -> int:
    if sector_id < 0 or sector_id >= sector_count:
        raise XlsFormatError("CFB sector is outside source bounds")
...
def _sector_bytes(
    ...
    *,
    ...
    if end > len(data):
        raise XlsFormatError("CFB sector is truncated")
...
def _walk_chain(
    ...
    *,
    ...
    while sector != _ENDOFCHAIN:
        if sector in seen:
            raise XlsFormatError(f"CFB {label} chain cycle detected")
        if sector in _REGULAR_SECTOR_MARKERS or sector >= sector_count:
            raise XlsFormatError(f"CFB {label} chain sector is outside bounds")
        if len(chain) >= max_sectors:
            raise XlsFormatError(f"CFB {label} chain exceeds resource limit")
        if sector >= len(table):
            raise XlsFormatError(f"CFB {label} chain table is truncated")
        seen.add(sector)
        chain.append(sector)
        next_sector = table[sector]
        if next_sector == _FREESECT:
            raise XlsFormatError(f"CFB {label} chain terminates in a free sector")
        if next_sector in {_FATSECT, _DIFSECT}:
            raise XlsFormatError(f"CFB {label} chain enters structural sector")
        sector = next_sector
    return tuple(chain)
...
def _read_regular_stream(
    ...
    *,
    ...
    if stream_size < 0 or stream_size > limits.max_stream_bytes:
        raise XlsFormatError(f"CFB {label} stream exceeds resource limit")
    if stream_size == 0:
        if start_sector not in {_ENDOFCHAIN, _FREESECT, 0}:
            raise XlsFormatError(f"CFB empty {label} stream has invalid start sector")
        return b"", (), ()
    ...
    if len(chain) != expected_sectors:
        raise XlsFormatError(f"CFB {label} stream chain size mismatch")
    ...
    for sector_id in chain:
        ...
        remaining -= take
    if remaining:
        raise XlsFormatError(f"CFB {label} stream is truncated")
...
def _read_ministream(
    *,
    ...
    if stream_size <= 0:
        raise XlsFormatError(f"CFB {label} mini stream has invalid size")
    ...
    while mini_sector != _ENDOFCHAIN:
        if mini_sector in seen:
            raise XlsFormatError(f"CFB MiniFAT {label} chain cycle detected")
        if mini_sector in _REGULAR_SECTOR_MARKERS or mini_sector >= mini_sector_count:
            raise XlsFormatError(f"CFB MiniFAT {label} sector is outside bounds")
        if mini_sector >= len(minifat_entries):
            raise XlsFormatError(f"CFB MiniFAT {label} table is truncated")
        if len(chain) >= limits.max_chain_sectors:
            raise XlsFormatError(f"CFB MiniFAT {label} chain exceeds resource limit")
        ...
        if next_sector in {_FATSECT, _DIFSECT}:
            raise XlsFormatError(f"CFB MiniFAT {label} chain uses invalid marker")
        mini_sector = next_sector

    expected_sectors = (stream_size + mini_sector_size - 1) // mini_sector_size
    if len(chain) != expected_sectors:
        raise XlsFormatError(f"CFB MiniFAT {label} chain size mismatch")
    ...
    for mini_sector_id in chain:
        ...
        if logical_start >= len(root_bytes):
            raise XlsFormatError(f"CFB MiniFAT {label} range is outside root stream")
        root_sector_index = logical_start // sector_size
        within_sector = logical_start % sector_size
        if root_sector_index >= len(root_chain):
            raise XlsFormatError(f"CFB MiniFAT {label} root chain is truncated")
        take = min(remaining, mini_sector_size)
        if logical_start + take > len(root_bytes):
            raise XlsFormatError(f"CFB MiniFAT {label} range is truncated")
        absolute_start = (
    ...
    *,
    ...
    if object_type == 0:
        ...
        if name_length not in {0, 2}:
            raise XlsFormatError("CFB empty directory entry has invalid name length")
    else:
        if name_length < 2 or name_length > 64 or name_length % 2:
            raise XlsFormatError("CFB directory entry has invalid name length")
        name_raw = raw[: name_length - 2]
        terminator = raw[name_length - 2 : name_length]
        if terminator != b"\x00\x00":
            raise XlsFormatError("CFB directory entry name is not terminated")
        try:
        ...
        if "\x00" in name:
            raise XlsFormatError("CFB directory entry name contains embedded NUL")
    if object_type not in {0, 1, 2, 5}:
        raise XlsFormatError("CFB directory entry has invalid object type")
...
def _assign_directory_parents(
    ...
    if not entries or entries[0].object_type != 5:
        raise XlsFormatError("CFB root directory entry is missing")
    ...
    def visit_tree(entry_id: int, parent_id: int) -> None:
        ...
        if entry_id >= len(entries):
            raise XlsFormatError("CFB directory sibling id is outside bounds")
        if entry_id in visiting:
            raise XlsFormatError("CFB directory topology cycle detected")
        entry = entries[entry_id]
        if entry.object_type == 0:
            raise XlsFormatError("CFB directory tree references an empty entry")
        existing_parent = parents.get(entry_id)
        if existing_parent is not None or entry_id in parents:
            if existing_parent != parent_id:
                raise XlsFormatError("CFB directory entry has ambiguous ownership")
            return
    ...
    for entry in entries[1:]:
        if entry.object_type != 0 and entry.directory_id not in parents:
            raise XlsFormatError("CFB directory contains unreachable allocated entry")
...
def parse_cfb(data: bytes, *, limits: XlsLimits | None = None) -> ParsedCfb:
    ...
    if len(source) > active_limits.max_source_bytes:
        raise XlsFormatError("CFB source exceeds resource limit")
    if len(source) < 512:
        raise XlsFormatError("CFB source is too small or truncated")
    if source[:8] != _CFB_SIGNATURE:
        raise XlsFormatError("CFB signature is invalid")
    ...
    if byte_order != 0xFFFE:
        raise XlsFormatError("CFB byte order is unsupported")
    if major_version == 3:
        if sector_shift != 9:
            raise XlsFormatError("CFB version 3 sector shift is invalid")
    elif major_version == 4:
        if sector_shift != 12:
            raise XlsFormatError("CFB version 4 sector shift is invalid")
    else:
        raise XlsFormatError("CFB major version is unsupported")
    if mini_sector_shift != 6:
        raise XlsFormatError("CFB mini sector shift is invalid")

    sector_size = 1 << sector_shift
    mini_sector_size = 1 << mini_sector_shift
    if len(source) < sector_size or (len(source) - sector_size) % sector_size:
        raise XlsFormatError("CFB source size is truncated or not sector aligned")
    if major_version == 4 and any(source[512:sector_size]):
        raise XlsFormatError("CFB version 4 extended header bytes are nonzero")
    ...
    if major_version == 3 and number_of_directory_sectors != 0:
        raise XlsFormatError("CFB version 3 directory-sector count must be zero")
    if mini_stream_cutoff != 0x1000:
        raise XlsFormatError("CFB mini stream cutoff is unsupported")
    if (
        number_of_fat_sectors <= 0
        or number_of_fat_sectors > active_limits.max_fat_sectors
    ):
        raise XlsFormatError("CFB FAT sector count exceeds resource limit")
    if number_of_difat_sectors > active_limits.max_difat_sectors:
        raise XlsFormatError("CFB DIFAT sector count exceeds resource limit")
    if number_of_minifat_sectors > active_limits.max_minifat_sectors:
        raise XlsFormatError("CFB MiniFAT sector count exceeds resource limit")
    ...
    for _ in range(number_of_difat_sectors):
        if next_difat in difat_sector_ids:
            raise XlsFormatError("CFB DIFAT chain cycle detected")
        raw = _sector_bytes(
            source,
            next_difat,
            sector_size=sector_size,
    ...
    if number_of_difat_sectors == 0:
        if first_difat_sector not in {_ENDOFCHAIN, _FREESECT}:
            raise XlsFormatError("CFB DIFAT start is inconsistent")
    elif next_difat != _ENDOFCHAIN:
        raise XlsFormatError("CFB DIFAT chain does not terminate")
    if len(fat_sector_ids) != number_of_fat_sectors:
        raise XlsFormatError("CFB FAT sector count does not match DIFAT")
    if len(set(fat_sector_ids)) != len(fat_sector_ids):
        raise XlsFormatError("CFB FAT sector ownership is duplicated")
    if set(fat_sector_ids) & set(difat_sector_ids):
        raise XlsFormatError("CFB FAT and DIFAT sectors overlap")
    ...
    if len(fat_entries) < sector_count:
        raise XlsFormatError("CFB FAT is truncated")
    for fat_sector_id in fat_sector_ids:
        if fat_entries[fat_sector_id] != _FATSECT:
            raise XlsFormatError("CFB FAT sector marker is invalid")
    for difat_sector_id in difat_sector_ids:
        if fat_entries[difat_sector_id] != _DIFSECT:
            raise XlsFormatError("CFB DIFAT sector marker is invalid")
    ...
    if not directory_chain:
        raise XlsFormatError("CFB directory chain is missing")
    if major_version == 4 and len(directory_chain) != number_of_directory_sectors:
        raise XlsFormatError("CFB directory sector count mismatch")
    ...
    if directory_slot_count > active_limits.max_directory_entries:
        raise XlsFormatError("CFB directory entry count exceeds resource limit")
    ...
    if number_of_minifat_sectors:
        ...
        if len(minifat_chain) != number_of_minifat_sectors:
            raise XlsFormatError("CFB MiniFAT sector count mismatch")
        ...
        raise XlsFormatError("CFB MiniFAT start is inconsistent")

    if root.stream_size > active_limits.max_total_owned_stream_bytes:
        raise XlsFormatError("CFB root mini stream exceeds resource limit")
    ...
    if set(root_chain) & structural_sector_ids:
        raise XlsFormatError("CFB root mini stream overlaps structural sectors")
    ...
    for entry in directory_entries:
        ...
        if entry.parent_id is None:
            raise XlsFormatError("CFB stream has no directory owner")
        if entry.stream_size > active_limits.max_stream_bytes:
            raise XlsFormatError("CFB stream exceeds resource limit")
        total_stream_bytes += entry.stream_size
        if total_stream_bytes > active_limits.max_total_owned_stream_bytes:
            raise XlsFormatError("CFB owned stream bytes exceed resource limit")

        if entry.stream_size < mini_stream_cutoff:
            if not minifat_entries:
                raise XlsFormatError("CFB mini stream exists without MiniFAT")
            ...
            if regular_sectors & structural_sector_ids:
                raise XlsFormatError("CFB stream overlaps structural sectors")
            if regular_sectors & set(root_chain):
                raise XlsFormatError("CFB stream overlaps root mini stream")
        ...
        for other in streams:
            if _ranges_overlap(stream.physical_ranges, other.physical_ranges):
                raise XlsFormatError("CFB stream physical ranges overlap or alias")
