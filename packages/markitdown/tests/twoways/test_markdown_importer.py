...
def test_image_alt_change_generates_set_alt_text():
    ...
    result = import_identity_markdown(edited, original_document=doc, manifest=projection.manifest)
...
def test_stale_document_digest_fails_closed():
    ...
    with pytest.raises(MarkdownIdentityError) as raised:
        import_identity_markdown(projection.markdown, original_document=stale, manifest=projection.manifest)
...
def test_missing_header_fails_closed():
    ...
    with pytest.raises(MarkdownIdentityError) as raised:
        import_identity_markdown(edited, original_document=doc, manifest=projection.manifest)
...
def test_missing_block_marker_is_not_remove_node():
    ...
    marker_line = next(line for line in projection.markdown.splitlines() if f'pid="{first.projection_id}"' in line)
...
def test_duplicate_projection_id_fails_closed():
    doc, projection = identity_fixture()
    marker = next(line for line in projection.markdown.splitlines() if "m2w:block" in line)
...
def test_unknown_projection_id_fails_closed():
    doc, projection = identity_fixture()
    first = projection.manifest.blocks[0]
    edited = projection.markdown.replace(f'pid="{first.projection_id}"', 'pid="p_unknown"', 1)
    with pytest.raises(MarkdownIdentityError) as raised:
        import_identity_markdown(edited, original_document=doc, manifest=projection.manifest)
...
def test_clean_manifest_cannot_be_identity_imported():
    ...
    with pytest.raises(MarkdownIdentityError):
        import_identity_markdown(projection.markdown, original_document=doc, manifest=projection.manifest)
...
def test_operation_ids_are_deterministic():
    doc, projection = identity_fixture()
    edited = projection.markdown.replace("Revenue increased", "Revenue grew")
    a = import_identity_markdown(edited, original_document=doc, manifest=projection.manifest)
    b = import_identity_markdown(edited, original_document=doc, manifest=projection.manifest)
...
def test_duplicate_node_assignment_fails_closed_before_guessing():
    ...
    second_marker = next(
        line for line in projection.markdown.splitlines()
        if f'pid="{second.projection_id}"' in line
    )
    edited_marker = second_marker.replace(f'node="{second.node_id}"', f'node="{first.node_id}"')
