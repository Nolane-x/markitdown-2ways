...
_XLSX_EVIDENCE_KEY = "twoways.xlsx_converter_snapshot.v1"
_XLSX_CONVERTER_BLOB_SHA = "355dd8f8d74ab5c9a40bba37e1f7a7d601eeba27"
_ACCEPTED_EXTENSIONS = frozenset({".xlsx"})
...
@dataclass(frozen=True)
class XlsxSheetMarkdownSnapshot:
    name: str
    markdown: str
...
@dataclass(frozen=True)
class XlsxConverterSnapshot:
    sheets: tuple[XlsxSheetMarkdownSnapshot, ...]
    provider: str
    materialization_id: str | None = None
    ...
    def __post_init__(self) -> None:
        ...
        for sheet in self.sheets:
            if not isinstance(sheet, XlsxSheetMarkdownSnapshot):
                raise TypeError("sheets must contain XlsxSheetMarkdownSnapshot values")
...
def _capture_source(source_stream: BinaryIO, *, max_bytes: int) -> bytes:
    ...
    while total <= max_bytes:
        ...
        if isinstance(chunk, str):
            raise TypeError("XlsxConverter source stream must return bytes")
        if not isinstance(chunk, (bytes, bytearray, memoryview)):
            raise TypeError("XlsxConverter source stream returned a non-bytes value")
        ...
        total += len(data)
        if total > max_bytes:
            raise ValueError("XlsxConverter source exceeds max_source_bytes")

    return b"".join(chunks)


def _project_markdown(snapshot: XlsxConverterSnapshot) -> str:
    markdown = ""
    ...
    return encode_capabilities(
        (
            CapabilityDecision(
                ...
                reason_code="xlsx.output.not_native_writable",
                constraints={
...
def read_xlsx_converter_snapshot_ir(
    source_stream: BinaryIO,
    *,
    stream_info: StreamInfo,
    snapshot: XlsxConverterSnapshot,
    limits: XlsxDerivedLimits | None = None,
) -> DocumentIR:
    if not isinstance(snapshot, XlsxConverterSnapshot):
        raise TypeError("snapshot must be an XlsxConverterSnapshot")

    accepted_by = _accepted_by(stream_info)
    if accepted_by is None:
        raise ValueError(
            "source is not owned by the explicit XlsxConverter extension/MIME "
            "acceptance surface"
        )

    active_limits = limits or XlsxDerivedLimits()
    ...
    identity_seed = "\0".join(
        (
            "xlsx-converter-derived-sheet-snapshot",
            source_digest,
            ...
            snapshot.materialization_path or "",
            *sheet_identity,
            markdown_digest,
            "XlsxConverter",
        )
    ...
    evidence: dict[str, object] = {
        ...
        "converter_blob_sha": _XLSX_CONVERTER_BLOB_SHA,
    ...
    if snapshot.materialization_path is not None:
        evidence["materialization_path"] = snapshot.materialization_path
    ...
    node = Node(
        ...
        provenance=(
            Provenance(
                source_format="xlsx-converter-derived",
                canvas_index=0,
                extraction_method="XlsxConverter-caller-materialization",
                metadata={
                    "source_sha256": source_digest,
                    "sheet_count": len(snapshot.sheets),
                    "markdown_sha256": markdown_digest,
                    "provider": snapshot.provider,
                    "materialization_path": snapshot.materialization_path,
                    "native_owner": False,
                },
            ),
        ),
        ...
        metadata={
            ...
            "twoways.xlsx_converter_snapshot.markdown_sha256": markdown_digest,
        },
    )
    ...
    document = DocumentIR(
        ...
        source=SourceDescriptor(
            format="xlsx-converter-derived-source",
        ...
        metadata=DocumentMetadata(custom={_XLSX_EVIDENCE_KEY: evidence}),
        canvases=(
            Canvas(
                canvas_id=canvas_id,
                index=0,
                kind="derived-xlsx-workbook",
                name=stream_info.filename or "XlsxConverter derived output",
        ...
        diagnostics=(
            Diagnostic(
                code="xlsx.output.not_native_writable",
                severity="info",
                message=(
                    "Materialized XlsxConverter workbook Markdown is derived and has "
                    "no H29 native write authority. Native XLSX reader/writer and "
                    "identity Markdown retain XLSX mutation authority."
                ),
                node_id=node_id,
                canvas_id=canvas_id,
                details={
