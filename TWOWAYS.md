...
## PdfConverter derived extraction parity

Phase H27 separates the unchanged one-way `PdfConverter` human-readable page extraction
from H9-H11's deliberately narrow native PDF mutation authority. The caller supplies
exact PDF source bytes plus already-materialized extraction text captured immediately
before the one-way converter's final partial-numbering post-process.

H27 independently reproduces `_merge_partial_numbering_lines`: a line matching
`^\.\d+# MarkItDown 2Ways
...
top-level Unicode `PidTagSubject` replacement.
 ...
 is merged with the next non-empty line using one ASCII space, skipped blank
lines disappear from that merge, terminal unmatched numbering is retained, and all other
lines preserve their sequence. No additional stripping or table reconstruction is
performed.

Production H27 imports and executes neither pdfminer nor pdfplumber. It performs no PDF
parsing, network or subprocess work. Source bytes, raw extraction text and final
Markdown are independently digested; the descriptive extraction path is explicitly
recorded as unverified provenance rather than native authority. The root is
`CapabilityState.DERIVED` with reason `pdf.output.not_native_writable`, has no native
locator, and exposes no writer.

`PdfDerivedLimits` independently bounds source, extracted UTF-8 text and final Markdown,
with source reads capped at 64 KiB. H9-H11 remain the only PDF native mutation paths for
their existing proven metadata, URI-link and terminal plain-text AcroForm owners.

## XlsConverter derived sheet parity

Phase H28 separates the unchanged one-way `XlsConverter` workbook Markdown projection
from H17's deliberately narrow native BIFF8 NUMBER-slot authority. The caller supplies
exact XLS source bytes plus an ordered tuple of already-materialized per-sheet Markdown
values representing the output of the external pandas/xlrd/DataFrame-to-HTML and
HtmlConverter stages before `XlsConverter` applies its own local strip/scaffold logic.

For every sheet in order H28 reproduces exactly:
...
The complete concatenation is then stripped exactly once, matching the unchanged
one-way converter. Raw per-sheet materialized Markdown remains separately digested, so
leading/trailing whitespace that does not survive visible projection still remains
provenance-bearing evidence. Sheet order and sheet names are identity-bearing.

Production H28 imports and executes neither pandas nor xlrd and performs no DataFrame,
HTML, BeautifulSoup, markdownify, network or subprocess work. The root is
`CapabilityState.DERIVED` with reason `xls.output.not_native_writable`, has no native
locator and exposes no writer. H17 remains the only XLS native mutation path and still
permits only existing proven BIFF8 NUMBER Xnum slots.

`XlsDerivedLimits` independently bounds source bytes, sheet count, aggregate raw
materialized UTF-8 Markdown and final workbook Markdown, with source reads capped at
