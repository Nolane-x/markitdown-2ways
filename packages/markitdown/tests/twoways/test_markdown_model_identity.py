...
from markitdown.twoways.markdown import projection_manifest_bytes, projection_manifest_digest
...
def test_projection_header_encoder_is_canonical():
    marker = encode_projection_header(version="1", document_id="doc1", source_digest="b" * 64)
    assert marker == '<!-- m2w:projection v="1" doc="doc1" base="sha256:' + "b" * 64 + '" -->'
...
def test_unknown_marker_key_fails_strict_mode():
    line = '<!-- m2w:block pid="p1" node="n1" kind="text" src="sha256:' + "a" * 64 + '" extra="x" -->'
    with pytest.raises(MarkdownIdentityError):
        parse_marker_line(line)
...
def test_marker_values_reject_physical_newlines():
    with pytest.raises(MarkdownIdentityError):
        encode_block_marker(projection_id="bad\nid", node_id="n1", kind="text", source_digest="a" * 64)


def test_manifest_serialization_is_deterministic():
    block = ProjectionBlock(
        projection_id="p1", node_id="n1", canvas_id="c1", node_kind="text",
        semantic_role=None, ordinal=0, source_semantic_digest="a" * 64,
        native_locator_digest=None, editable_capabilities=("replace_text",),
        rendered_digest="b" * 64,
    )
    manifest = ProjectionManifest(
        format_version="1", document_id="doc1", document_schema_version="0.1.0",
        source_document_digest="c" * 64, projection_mode="identity", blocks=(block,),
    )
...
def test_projection_block_rejects_invalid_digest_shape():
    with pytest.raises(ValueError, match="source_semantic_digest"):
        ProjectionBlock(
            projection_id="p1", node_id="n1", canvas_id=None, node_kind="text",
            semantic_role=None, ordinal=0, source_semantic_digest="not-a-digest",
            native_locator_digest=None, editable_capabilities=(), rendered_digest="b" * 64,
        )
