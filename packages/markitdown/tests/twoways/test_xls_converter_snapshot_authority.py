...
def test_workbook_snapshot_descriptors_reject_blank_values(
    ...
    values[field] = value
    with pytest.raises(ValueError, match=field):
        XlsConverterSnapshot(**values)


def test_workbook_snapshot_requires_non_empty_tuple_and_unique_names() -> None:
    with pytest.raises(TypeError, match="tuple"):
        XlsConverterSnapshot(sheets=list(SHEETS), provider="fixture")

    with pytest.raises(ValueError, match="non-empty"):
        XlsConverterSnapshot(sheets=(), provider="fixture")

    duplicate = (
        XlsSheetMarkdownSnapshot(name="A", markdown="one"),
        XlsSheetMarkdownSnapshot(name="A", markdown="two"),
    )
    with pytest.raises(ValueError, match="unique"):
        XlsConverterSnapshot(sheets=duplicate, provider="fixture")
...
def test_source_limit_accepts_exact_boundary_and_reads_bounded() -> None:
    ...
    exact = read_xls_converter_snapshot_ir(
        probe,
        stream_info=INFO,
        snapshot=_snapshot(),
        limits=XlsDerivedLimits(max_source_bytes=len(SOURCE)),
    )
    ...
    with pytest.raises(ValueError, match="max_source_bytes"):
        read_xls_converter_snapshot_ir(
            ...
            stream_info=INFO,
            snapshot=_snapshot(),
            limits=XlsDerivedLimits(max_source_bytes=len(SOURCE) - 1),
        )
...
def test_sheet_count_and_markdown_limits_have_exact_boundaries() -> None:
    ...
    with pytest.raises(ValueError, match="max_sheet_count"):
        read_xls_converter_snapshot_ir(
            ...
            stream_info=INFO,
            snapshot=_snapshot(),
            limits=XlsDerivedLimits(max_sheet_count=len(SHEETS) - 1),
        )
    ...
    with pytest.raises(ValueError, match="max_materialized_utf8_bytes"):
        read_xls_converter_snapshot_ir(
            ...
            stream_info=INFO,
            snapshot=_snapshot(),
            limits=XlsDerivedLimits(max_materialized_utf8_bytes=raw_size - 1),
        )
    ...
    with pytest.raises(ValueError, match="max_markdown_utf8_bytes"):
        read_xls_converter_snapshot_ir(
            ...
            stream_info=INFO,
            snapshot=_snapshot(),
            limits=XlsDerivedLimits(max_markdown_utf8_bytes=markdown_size - 1),
        )


def test_snapshot_must_use_explicit_type() -> None:
    with pytest.raises(TypeError, match="XlsConverterSnapshot"):
        read_xls_converter_snapshot_ir(
            BytesIO(SOURCE),
            stream_info=INFO,
            snapshot={"sheets": []},
